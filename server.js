    ...
    db.serialize(() => {
        const tables = [
            ...
            `CREATE TABLE IF NOT EXISTS users (
                 ...
                 profilePic TEXT,
                 location TEXT,
                 status TEXT NOT NULL DEFAULT 'Active','banned','suspended',
                 lastLogin TEXT,
                 created_at TEXT DEFAULT CURRENT_TIMESTAMP,
                 phone TEXT,
                 address TEXT
            ...
            `CREATE TABLE IF NOT EXISTS app_settings (
             ...
             `CREATE TABLE IF NOT EXISTS announcements (
                 ...
                 description TEXT,
                 callButtonText TEXT NOT NULL,
                 callButtonPhone TEXT,
                 whatsappButtonText TEXT NOT NULL,
                 whatsappButtonPhone TEXT NOT NULL,
                 backgroundImageUrl TEXT,
                 backgroundColor TEXT DEFAULT '#f97316',
    ...
    db.run('INSERT INTO snacks (id, name, price, stock, imageUrl, category, description, discount, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
        [id, name, price || 0, stock || 0, imageUrl || '', category || 'Uncategorized', description || '', discount || 0, created_at, updated_at], function (err) {
            ...
            io.emit('snack-notification', {
                ...
                message: `${name} is now available for UGX ${price || 0}`,
                snack: newSnack
            });
...
// MarzPay Payment Integration - Initiate Payment in UGX
app.post('/api/payments/marzpay/initiate', async (req, res) => {
    try {
        ...
        if (!userEmail || !userPhone) {
            return res.status(400).json({ 
                error: 'Missing required fields: userEmail and userPhone are required for MarzPay payments' 
            });
        }

        // Basic email and phone format checks
        const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
        const phoneClean = String(userPhone).replace(/[^0-9+]/g, '');
        ...
        // Convert amount to UGX (ensure it's an integer)
        const amountInUGX = Math.round(parseFloat(amount) * 1);
        ...
        const paymentPayload = {
            ...
            redirect_url: process.env.REDIRECT_URL || 'http://localhost:3000/index.html',
            webhook_url: `${process.env.API_BASE_URL || 'http://localhost:5505'}/api/payments/marzpay/webhook`,
            metadata: {
                userId: userId,
                orderId: orderId,
        ...
        // Make request to MarzPay API
        const fetch = (await import('node-fetch')).default;
        const marzpayResponse = await fetch(`${MARZPAY_BASE_URL}/v1/payment_intents`, {
            method: 'POST',
            headers: {
        ...
        const responseData = await marzpayResponse.json();

        // Check if request was successful
        if (!marzpayResponse.ok) {
            console.error('MarzPay API Error:', responseData);
            return res.status(marzpayResponse.status).json({
                error: 'Failed to create payment intent with MarzPay',
        ...
        db.run(
            `INSERT INTO payments (id, orderId, amount, method, status, transactionId, timestamp) 
             VALUES (?, ?, ?, ?, ?, ?, ?)`,
            [paymentId, orderId, amountInUGX, 'marzpay', 'initiated', responseData.id, timestamp],
            function (err) {
                if (err) {
        ...
        console.error('Error initiating MarzPay payment:', error);
        res.status(500).json({
...
// MarzPay Webhook - Handle payment status updates
app.post('/api/payments/marzpay/webhook', (req, res) => {
    try {
        const { id, status, order_id, amount } = req.body;

        console.log('MarzPay Webhook received:', { id, status, order_id, amount });

        // Verify webhook is from MarzPay (in production, verify signature)
        if (!id || !status || !order_id) {
            return res.status(400).json({ error: 'Invalid webhook payload' });
        }

        // Update payment status in database
        const paymentStatus = status === 'succeeded' ? 'completed' : 
            ...
            function (err) {
                ...
                if (status === 'succeeded') {
                    db.run(
                        ...
                        [order_id],
                        function (err) {
                            if (err) console.error('Error updating order:', err.message);
                        }
                    );
                }
        ...
        console.error('Error handling MarzPay webhook:', error);
        res.status(500).json({ error: 'Webhook processing failed' });
    }
});

// MarzPay Payment Status Check
app.get('/api/payments/marzpay/status/:paymentIntentId', async (req, res) => {
    try {
        ...
        const fetch = (await import('node-fetch')).default;
        const marzpayResponse = await fetch(`${MARZPAY_BASE_URL}/v1/payment_intents/${paymentIntentId}`, {
            method: 'GET',
            headers: {
                'Authorization': `Basic ${authString}`,
                'Content-Type': 'application/json',
        ...
        res.json({
            ...
            orderId: responseData.metadata?.order_id,
            createdAt: responseData.created_at
        });

    } catch (error) {
        console.error('Error fetching MarzPay payment status:', error);
        res.status(500).json({
...
app.get('/api/cart/:userId', (req, res) => {
    const userId = req.params.userId;
    db.get('SELECT * FROM user_carts WHERE userId = ?', [userId], (err, row) => {
        if (err) return res.status(500).json({ error: err.message });
        
        if (!row) {
            return res.json({ items: [], discountCode: '' });
        }
...
app.post('/api/cart/:userId', (req, res) => {
    ...
    db.get('SELECT id FROM user_carts WHERE userId = ?', [userId], (err, row) => {
        if (err) return res.status(500).json({ error: err.message });
        
        if (row) {
            // Update existing cart
            db.run('UPDATE user_carts SET items = ?, discountCode = ?, updated_at = ? WHERE userId = ?',
                [JSON.stringify(items), discountCode || '', now, userId], function (err) {
                    if (err) return res.status(500).json({ error: err.message });
                    res.json({ message: 'Cart updated successfully' });
                });
        } else {
            // Create new cart
            db.run('INSERT INTO user_carts (id, userId, items, discountCode, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?)',
                [id, userId, JSON.stringify(items), discountCode || '', now, now], function (err) {
                    if (err) return res.status(500).json({ error: err.message });
...
app.delete('/api/cart/:userId', (req, res) => {
    const userId = req.params.userId;
    db.run('DELETE FROM user_carts WHERE userId = ?', [userId], function (err) {
        if (err) return res.status(500).json({ error: err.message });
...
app.get('/api/announcements', (req, res) => {
    db.all('SELECT * FROM announcements WHERE isActive = 1 ORDER BY created_at DESC LIMIT 1', [], (err, rows) => {
        if (err) return res.status(500).json({ error: err.message });
...
app.post('/api/announcements', (req, res) => {
    ...
    if (!title || !mainText || !callButtonText || !whatsappButtonText || !whatsappButtonPhone) {
        return res.status(400).json({ error: 'Required fields: title, mainText, callButtonText, whatsappButtonText, whatsappButtonPhone' });
    }
    ...
    db.run('UPDATE announcements SET isActive = 0', (err) => {
        if (err) return res.status(500).json({ error: err.message });

        // Insert new announcement
        db.run(`INSERT INTO announcements (
            id, title, subtitle, mainText, description, callButtonText, callButtonPhone,
            whatsappButtonText, whatsappButtonPhone, backgroundImageUrl, backgroundColor,
            textColor, isActive, created_at, updated_at
        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
        [
            id, title, subtitle, mainText, description, callButtonText, callButtonPhone,
            whatsappButtonText, whatsappButtonPhone, backgroundImageUrl, backgroundColor,
            textColor, isActive !== undefined ? isActive : 1, now, now
        ],
        function (err) {
            if (err) return res.status(500).json({ error: err.message });
            res.json({ id, message: 'Announcement created successfully' });
        });
    });
});
...
app.put('/api/announcements/:id', (req, res) => {
    ...
    db.run(`UPDATE announcements SET
        title = ?, subtitle = ?, mainText = ?, description = ?, callButtonText = ?,
        callButtonPhone = ?, whatsappButtonText = ?, whatsappButtonPhone = ?,
        backgroundImageUrl = ?, backgroundColor = ?, textColor = ?, isActive = ?,
        updated_at = ?
        WHERE id = ?`,
    [
        title, subtitle, mainText, description, callButtonText, callButtonPhone,
        whatsappButtonText, whatsappButtonPhone, backgroundImageUrl, backgroundColor,
        textColor, isActive !== undefined ? isActive : 1, now, id
    ],
    function (err) {
        if (err) return res.status(500).json({ error: err.message });
        res.json({ id, message: 'Announcement updated successfully' });
    });
});
...
io.on('connection', (socket) => {
    ...
    socket.on('new-snack-added', (snackData) => {
        ...
        io.emit('snack-notification', {
            type: 'new_snack',
            title: 'New Snack Available!',
            message: `${snackData.name} is now available for UGX ${snackData.price}`,
            snack: snackData
        });
    });
