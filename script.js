...
// Search and sort state for snacks
let snackSearchTerm = '';
let snackSortBy = 'name-asc';
let filteredSnacks = [];

// Message status tracking
let messageStatuses = {}; // { messageId: 'sent' | 'delivered' | 'read' }
...
const initializeSocket = () => {
    ...
    socket.on('message-sent', (messageData) => {
        ...
        // Refresh message display if currently viewing messages
        if (currentView === 'messages' && selectedUserForMessaging) {
            setTimeout(() => {
                loadMessageHistory(selectedUserForMessaging);
            }, 100);
        }
    });

    // Handle message delivery status updates
    socket.on('message-status-update', (statusData) => {
...
window.onload = async () => {
    ...
    showMessage(`Dashboard ready! Welcome Bitbyte`, 'info');
...
// Message status icon helper function
const getMessageStatusIcon = (status) => {
    switch (status) {
        case 'read':
            return `<i data-lucide="check-check" class="w-4 h-4 ml-2 text-blue-500" title="Read"></i>`;
        case 'sent':
        default:
            return `<i data-lucide="check" class="w-4 h-4 ml-2 text-blue-300" title="Sent"></i>`;
    }
};
...
const updateMessageStatus = (messageId, status) => {
    ...
    // If message is read, also update the database
    if (status === 'read') {
...
const renderSnackManagement = () => {
    // Initialize search state when entering this view
    if (currentView === 'snacks') {
        // Set default search term if empty
        if (!snackSearchTerm) snackSearchTerm = '';
        // Set default sort if not set
        if (!snackSortBy) snackSortBy = 'name-asc';
    }
    ...
    return `
                ...
                <div class="flex flex-col h-full space-y-6">
                    ...
                    <div class="flex-shrink-0">
                        <div class="bg-white rounded-xl shadow-lg p-6">
                            <!-- Flexbox container for search controls -->
                            <div class="flex flex-col lg:flex-row gap-4 items-start lg:items-end">
                                <!-- Search Box Section -->
                                <div class="flex-1 w-full lg:w-auto">
                                    <label for="snack-search" class="block text-sm font-medium text-gray-700 mb-2">Search Snacks</label>
                                    <div class="relative">
                                        ...
                                        <input 
                                            ...
                                            onkeydown="handleSnackSearch(event)" 
                                    ...
                                    <p class="text-xs text-gray-500 mt-1">Press Enter or type to search by name, category, or date created</p>
                                </div>
                                
                                <!-- Sort Options Section -->
                                <div class="flex-shrink-0 w-full lg:w-auto">
                                    <label for="snack-sort" class="block text-sm font-medium text-gray-700 mb-2">Sort By</label>
                                    <select id="snack-sort" onchange="handleSnackSort()" class="block w-full lg:w-48 px-3 py-2 border border-gray-300 rounded-lg focus:ring-primary focus:border-primary">
                                        <option value="name-asc" ${snackSortBy === 'name-asc' ? 'selected' : ''}>Name (A-Z)</option>
                                        <option value="name-desc" ${snackSortBy === 'name-desc' ? 'selected' : ''}>Name (Z-A)</option>
                                        <option value="date-desc" ${snackSortBy === 'date-desc' ? 'selected' : ''}>Latest First</option>
                                        <option value="date-asc" ${snackSortBy === 'date-asc' ? 'selected' : ''}>Oldest First</option>
                                        <option value="category-asc" ${snackSortBy === 'category-asc' ? 'selected' : ''}>Category (A-Z)</option>
                                        <option value="category-desc" ${snackSortBy === 'category-desc' ? 'selected' : ''}>Category (Z-A)</option>
                                        <option value="price-asc" ${snackSortBy === 'price-asc' ? 'selected' : ''}>Price (Low to High)</option>
                                        <option value="price-desc" ${snackSortBy === 'price-desc' ? 'selected' : ''}>Price (High to Low)</option>
                                        <option value="stock-desc" ${snackSortBy === 'stock-desc' ? 'selected' : ''}>Stock (High to Low)</option>
                                        <option value="stock-asc" ${snackSortBy === 'stock-asc' ? 'selected' : ''}>Stock (Low to High)</option>
                                    </select>
                                </div>
                            </div>
                            ...
                            <div class="mt-4 flex justify-between items-center">
                                <div id="search-results-info" class="text-sm text-gray-600">
                                    <!-- Will be populated by updateSearchResultsInfo -->
                                </div>
                                <button 
                                    id="clear-search-btn" 
                                    onclick="clearSnackSearch()" 
                                    class="hidden text-sm text-primary hover:text-orange-600 font-medium"
                                >
                                    Clear Search
                                </button>
                            </div>
                    ...
                    <div class="flex-1 bg-white rounded-xl shadow-lg overflow-hidden min-h-0">
                        ${renderSnacksTable()}
                    </div>
                </div>
                
                <script>
                    // Initialize search results info when view loads
                    setTimeout(() => {
...
const renderSnacksTable = () => {
    // Apply search and sort to snacks array
    const processedSnacks = getProcessedSnacks();
    
    if (processedSnacks.length === 0) {
        const searchText = snackSearchTerm ? ` matching "${snackSearchTerm}"` : '';
        return `<p class="p-6 text-center text-gray-500">No snacks found${searchText}. Try adjusting your search or sort criteria.</p>`;
    }
    ...
    const formContent = `
                <form id="snack-form" class="space-y-4">
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            <label for="snack-price" class="block text-sm font-medium text-gray-700">Price (UGX)</label>
...
const saveSnack = async () => {
    ...
    try {
        ...
        const snackData = {
            ...
            createdAt: existingSnack?.createdAt || getTimestamp()
        };
        ...
        // Maintain search state by re-rendering the snack management view
        if (currentView === 'snacks') {
            renderApp();
        }
    } catch (error) {
...
// Search and Sort Functionality for Snacks
const getProcessedSnacks = () => {
    let processed = [...snacks];
    
    // Apply search filter
    if (snackSearchTerm.trim()) {
        const searchLower = snackSearchTerm.toLowerCase().trim();
        processed = processed.filter(snack => {
            const nameMatch = snack.name.toLowerCase().includes(searchLower);
            const categoryMatch = (snack.category || '').toLowerCase().includes(searchLower);
            const createdAt = snack.created_at || snack.createdAt || snack.date || Date.now();
            const dateMatch = new Date(createdAt).toLocaleDateString().includes(searchLower) || 
                             new Date(createdAt).toLocaleString().toLowerCase().includes(searchLower);
            const priceMatch = snack.price.toString().includes(searchLower);
            const stockMatch = snack.stock.toString().includes(searchLower);
            const descriptionMatch = (snack.description || '').toLowerCase().includes(searchLower);
            
            return nameMatch || categoryMatch || dateMatch || priceMatch || stockMatch || descriptionMatch;
        });
    }
    ...
    processed.sort((a, b) => {
        switch (snackSortBy) {
            ...
            case 'date-desc':
            default:
                const dateADesc = new Date(a.created_at || a.createdAt || a.date || 0);
                const dateBDesc = new Date(b.created_at || b.createdAt || b.date || 0);
...
const handleSnackSearch = (event) => {
    ...
    snackSearchTerm = searchInput.value;
    
    // Update search results info and table
    updateSearchResultsInfo();
    rerenderSnacksTable();
};

const handleSnackSort = () => {
    const sortSelect = document.getElementById('snack-sort');
    if (!sortSelect) return;
    
    snackSortBy = sortSelect.value;
...
const updateSearchResultsInfo = () => {
    ...
    const processedSnacks = getProcessedSnacks();
    const totalSnacks = snacks.length;
    const showingResults = snackSearchTerm.trim().length > 0;
    
    if (showingResults) {
        searchInfo.textContent = `Showing ${processedSnacks.length} of ${totalSnacks} snacks matching "${snackSearchTerm}"`;
        if (clearButton) {
            clearButton.classList.remove('hidden');
        }
    } else {
        searchInfo.textContent = `Showing all ${totalSnacks} snacks`;
        if (clearButton) {
            clearButton.classList.add('hidden');
        }
    }
...
const rerenderSnacksTable = () => {
    const tableContainer = document.querySelector('.bg-white.rounded-xl.shadow-lg.overflow-x-auto');
    if (!tableContainer) return;
    
    // Re-render just the table part
    tableContainer.innerHTML = renderSnacksTable();
...
const renderSnackInventoryMonitor = () => {
    ...
    return `
                ...
                <div class="flex flex-col h-full space-y-6">
                    <!-- Header Section -->
                    <div class="flex-shrink-0">
                        <h3 class="text-2xl font-semibold">Snack Inventory Cards (${snacks.length})</h3>
                        <p class="text-sm text-gray-600 mt-2">Scroll right to view all items. Use the toggle button to quickly mark items in or out of stock.</p>
                    </div>
                    
                    <!-- Horizontal Scrollable Cards Container -->
                    <div class="flex-1 min-h-0">
                        <div class="h-full bg-white rounded-xl shadow-lg border border-gray-100 p-4">
                            <div class="flex overflow-x-auto h-full space-x-6 pb-4">
                                ${snacks.map(snack => `
                                    <div class="flex-none w-72 bg-gray-50 rounded-xl shadow-md overflow-hidden hover:shadow-xl transition duration-300 transform hover:-translate-y-0.5 self-start">
...
const loadMessageHistory = async (userId) => {
    try {
        ...
        messageHistory.innerHTML = messages.map(msg => {
            ...
            const messageStatus = msg.isRead ? 'read' : (messageStatuses[msg.id] || 'sent');
            
            return `
                        <div class="flex ${isFromAdmin ? 'justify-end' : 'justify-start'} mb-4">
                            <div class="max-w-xs lg:max-w-md px-4 py-2 rounded-lg ${isFromAdmin ? 'bg-primary text-white' : 'bg-gray-200 text-gray-800'} ${!isRead && !isFromAdmin ? 'cursor-pointer hover:bg-opacity-80' : ''}" 
                                 ${!isRead && !isFromAdmin ? `onclick="updateMessageStatus('${msg.id}', 'read')" title="Click to mark as read"` : ''}>
                                ${msg.subject ? `<div class="font-semibold text-sm mb-1">${msg.subject}</div>` : ''}
                                <div class="text-sm">${msg.content}</div>
                                <div class="flex items-center justify-between mt-2">
                                    <div class="text-xs opacity-70">${new Date(msg.timestamp).toLocaleString()}</div>
                                    ${isFromAdmin ? getMessageStatusIcon(messageStatus) : ''}
                                </div>
                                ${!isRead && !isFromAdmin ? '<div class="text-xs mt-1 opacity-70">(Click to mark as read)</div>' : ''}
                            </div>
                        </div>
                    `;
        }).join('');
        ...
        // Refresh Lucide icons to show status indicators
        lucide.createIcons();
    } catch (error) {
...
class ShoppingCart {
    ...
    render() {
        ...
        cartItemsList.innerHTML = this.items.map(item => `
            <div class="cart-item">
                ...
                <div class="cart-item-controls">
                    ...
                    <button class="cart-item-remove" onclick="window.cart.removeItem('${item.id}')">&times;</button>
                </div>
            </div>
        `).join('');
        ...
        if (cartTax) cartTax.textContent = formatCurrency(totals.tax);
